...
def getmediaUrl(mediaArgs):
    ...
    if mediaArgs[2] != "":
      query = "site:divxplanet.com inurl:sub/s intitle:\"%s\" intitle:\"(%s)\"" % (title, mediaArgs[2])
    else:
      query = "site:divxplanet.com inurl:sub/s intitle:\"%s\"" % ( title)
    ...
    for li in soup.findAll('li', attrs={'class':'g'}):
        ...
        if sLink:
            linkurl = re.search(r"\/url\?q=(http:\/\/divxplanet.com\/sub\/s\/[0-9]{3,8}\/.*.\.html).*", sLink["href"])
            if linkurl:
...
#def search_subtitles(tvshow, year, season, episode, title): #standard input
    ...
    if len(tvshow) != 0:
        ...
        if tvurl != "":
          divpname = re.search(r"http:\/\/divxplanet.com\/sub\/s\/[0-9]{3,8}\/(.*.)\.html", tvurl).group(1)
          season = int(season)
          episode = int(episode)
          # Browser
          br = mechanize.Browser()
